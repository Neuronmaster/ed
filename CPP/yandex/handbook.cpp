...
//     std::string word;
//     std::cin >> word;
//     std::string string_change = word.substr(0, 1);
...
//         if ((char_to_digit.find(c) != char_to_digit.end())&& (char_to_digit[c] != string_change.back())) {
//             string_change += char_to_digit[c];
//         }
//     }

//     // Обрезаем или добавляем нули, чтобы получить строку длиной 4 символа
//     if (string_change.length() < 4) {
//         string_change.append(4 - string_change.length(), '0');
//     } else if (string_change.length() > 4) {
//         string_change = string_change.substr(0, 4);
//     }
...
// k
// ​
//   — это номер места, на которое сел 
// 𝑘
// k-й гость.
...
// int n;
// std::cin >> n;
// std::vector<int> guests(n+1);
// std::vector<int> back_guests(n+1); // обратная перестановка

// for (int i = 1; i < n+1; i++) {
//     std::cin >> guests[i];
//     back_guests[guests[i]] = i;
// }
// for (int i = 1; i < n+1; i++) {
//     std::cout << back_guests[i] << " ";  // вывод обратной перестановки 
// }
// std::cout << std::endl;
// return 0;
// }
